...
const seedDatabase = async () => {
    try {
        ...
        let lastChapter = "Ray Optics & Optical Instruments";

        for (let i = 0; i < allQuestionsRaw.length; i++) {
            let q = allQuestionsRaw[i];
            let questionText = q.question;
            let chapterName = q.chapter;
            
            // Handle corrupted generator output where question is missing and placed in chapter
            if (!questionText) {
